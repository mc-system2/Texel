...
/**
 * prompt-studio.delete-patch.js
 * Drop-in module to enable BLOB deletion via DeletePromptText.
 * Load this file AFTER prompt-studio.js.
 */
(function(){
  "use strict";
  const log = (...a)=>{ try{ console.log("[delete-patch]", ...a);}catch{} };
  const $ = (s,r)=> (r||document).querySelector(s);

  async function apiDeletePromptBlob(blobPath, etag){
    const base = (window.FUNCTION_BASE || $("#apiBase")?.value || "").replace(/\/?$/,"");
    if (!base) throw new Error("FUNCTION_BASE/apiBase 未設定");
    const res = await fetch(base + "/DeletePromptText", {
      method: "POST",
  ...
  async function removeFromIndex(path){
    if (typeof removeEntryFromIndexAndSave === "function"){
      await removeEntryFromIndexAndSave(path);
      return;
    }
    // Fallback: fire event and let main handle
    document.dispatchEvent(new CustomEvent("prompt:index-remove", { detail:{ path } }));
  }

  document.addEventListener("click", async (ev)=>{
    const btn = ev.target.closest('[data-action="delete-prompt"], .prompt-trash, .btn-trash');
    if (!btn) return;
    ev.preventDefault(); ev.stopPropagation();

    const holder = btn.closest("[data-path]") || btn;
    const path = holder?.dataset?.path || btn?.dataset?.path;
    const name = holder?.dataset?.name || btn?.dataset?.name || path;
    const etag = holder?.dataset?.etag || btn?.dataset?.etag || undefined;
    if (!path) return;

    if (!confirm(`「${name}」を削除します。BLOB本体も削除されます。よろしいですか？`)) return;

    try{
      if (typeof setStatus === "function") setStatus("サーバから削除中…","orange");
      const del = await apiDeletePromptBlob(path, etag);
      if (!del.ok){
        if (del.status === 412){
          alert("削除できません（ETag不一致）。一度再読み込み後にやり直してください。");
          if (typeof setStatus === "function") setStatus("削除中断（ETag不一致）","red");
          return;
        }
        alert(`削除に失敗しました（${del.status||'unknown'}）。${del.detail||''}`);
        if (typeof setStatus === "function") setStatus("削除失敗","red");
        return;
      }
      if (typeof setStatus === "function") setStatus("BLOB削除成功。インデックス更新中…","green");
      await removeFromIndex(path);

      // UI からも除去
      const row = document.querySelector(`[data-path="${CSS.escape(path)}"]`);
      if (row) row.remove();
      if (window.currentEditing && window.currentEditing.path === path && typeof clearEditor === "function"){
        clearEditor();
      }
      if (typeof setStatus === "function") setStatus("削除しました。","green");
      if (typeof toast === "function") toast("削除しました");
    }catch(err){
      console.error(err);
      if (typeof setStatus === "function") setStatus("削除中にエラー: " + (err?.message||err),"red");
    }
  });
})();