...
async function apiPost(fn, body) {
    ...
    if (!base) {
        throw new Error('API Base 未設定');
    }
    const url = base.replace(/\/+$/, '') + '/' + fn.replace(/^\/+/, '');
    // ensure single slash
...
function getApiBase() {
    ...
    if (fromHash) {
        localStorage.setItem('clientEditor.apiBase', fromHash);
        return fromHash;
    }
...
function setApiBase(v) {
    ...
    if (v) {
        localStorage.setItem('clientEditor.apiBase', v);
    }
}
if (apiBaseInput) {
    setApiBase(getApiBase());
    apiBaseInput.addEventListener('change', e => setApiBase(e.target.value.trim()));
}
...
async function saveCatalog() {
    ...
    if (!v.ok) {
        showAlert(v.message || "入力エラーがあります。", "error");
        return;
    }
    ...
    try {
        ...
        if (!res.ok)
            throw new Error(json?.error || rawText || `HTTP ${res.status}`);
        ...
        previousCatalogCodes = new Map();
        for (const c of clients)
            previousCatalogCodes.set(c.code, normalizeBehavior(c.behavior || ""));
    } catch (e) {
...
function validateGrid() {
    ...
    inputs.forEach( (inp) => {
        ...
        if (!isFormatOk) {
            ...
            hint.style.display = "block";
            hasError = true;
            if (!duplicateList.includes(v))
                duplicateList.push(v);
        } else {
            hint.textContent = "";
            hint.style.display = "none";
        }
    ...
    if (hasError) {
        if (duplicateList.length)
            message += `重複: ${duplicateList.join(", ")} `;
    }
    if (sheetMissing)
        message += (message ? "/ " : "") + "Spreadsheet ID が未入力の行があります";
    setStatus(message);
    return {
        ok: !(hasError || sheetMissing),
        message
    };
}
...
function issueNewCode() {
    ...
    let n = 0;
    while (used.has(`Z${String(n).padStart(3, "0")}`))
        n++;
...
window.addEventListener("DOMContentLoaded", async () => {
    ...
    if (!els.apiBase.value) {
        ...
        // ★ここに本番SWAのホスト名を入れておく
        const isProdHost = host.includes("lemon-beach") || // texel の PROD 静的Webアプリ
        host.includes("texel-prod");
        // 予備：将来名前を変えたとき用

        const defaultBase = isProdHost ? PROD_API : DEV_API;
        setApiBase(defaultBase);
        // input と localStorage の両方を更新
    }
